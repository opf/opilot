...
//
// Every operation GitHub's /readonly endpoint offers. MUST match
// GH_READ_ONLY_OPS in mcp-gw.js — the gateway is the authority; this copy only
// shapes the schema the model sees, and the test asserts the two agree.
...
// The union of every flat field those operations declare. Flat rather than one
// opaque `arguments` blob for op-mcp-client.js's reason: the model sees the
// real parameter names instead of guessing at a nested object.
const FLAT_ARG_NAMES = Object.freeze([
  'affiliation', 'after', 'author', 'base', 'detail', 'direction',
  'field_filters', 'fields', 'head', 'issue_number', 'labels', 'method',
  'minimal_output', 'name', 'order', 'orderBy', 'owner', 'page', 'path',
  'perPage', 'pullNumber', 'query', 'ref', 'repo', 'sha', 'since', 'sort',
  'state', 'tag', 'until',
]);
...
const NUMERIC_ARG_NAMES = Object.freeze(['issue_number', 'page', 'perPage', 'pullNumber']);
