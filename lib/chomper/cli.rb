module Chomper
  # Argument parsing and dispatch for `./chomper <command>`. Everything a command
  ...
  class CLI
    ...
    def run(argv)
      ...
      # --help anywhere, not only as the first argument: `./chomper wp ship --help`
    ...
    # A group answers for itself, so `./chomper wp --help` doesn't print the
    ...
    # `agent` is both a group and chomper's main entry point: bare `./chomper
    ...
    def wp(args)
      # A bare `./chomper wp` is a request for help, so answer it before
    ...
    # GitHub PR URL (a URL is resolved to its WP via chomper's own state, else via
    ...
    def usage!(name, arg_spec, example = nil)
      $stderr.puts "Usage: ./chomper #{name} #{arg_spec}#{example ? "   (#{example})" : ""}"
      raise Chomper::FatalError
    end
