...
Guidance for `lib/chomper/pd/` (namespace `Chomper::PD`). The root `CLAUDE.md`
...
`lib/chomper/pd.rb` is the load boundary, and it is load-bearing: nothing here is
...
- **`pd init <project-id>`** — resolves ids **by name** (`ResolvedIds`): project,
  parent/child WP types (`CHOMPER_PD_PARENT_TYPE`/`CHOMPER_PD_CHILD_TYPE`, default
...
- **`pd implement <wp-id>...`** — builds one generated work package from its spec:
  ...
  without them a pd PR would be the one chomper PR that can't explain itself.
...
This is the one place chomper parses attacker-influenceable binary input inside the
...
conventions, so the format would otherwise live only in chomper's prompt — and
...
`CHOMPER_PD_PARENT_TYPE` / `CHOMPER_PD_CHILD_TYPE` (default `FEATURE` /
`IMPLEMENTATION`) and `CHOMPER_PD_IMPLEMENTING_STATUS` /
`CHOMPER_PD_IMPLEMENTED_STATUS` (default `In progress` / `Developed`) — all resolved
