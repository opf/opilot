...
module OPilot
  module Clients
    ...
    class GitHub
      ...
      # every product repo opilot targets (`dev` for openproject, `main`/`master`
      ...
      # opilot pushes branches to this fork and opens the PR against upstream,
      ...
      #
      # A fork's branch is frozen at fork-creation time while opilot's clones
      ...
      # Post a review carrying inline `suggestion` comments on a PR opilot does
      # not own (`event: COMMENT` — opilot never approves or blocks). `comments`
      ...
      # resolve against the diff opilot actually read. No push access needed.
      ...
      # opilot saw it before it starts working. `kind` is :issue (conversation
      ...
      # All used only by gh-agent's CI-fix path (opilot's own PRs). Each returns
      ...
      # The PR number embedded in a opilot-stored PR URL
