...
module OPilot
  module PD
    ...
    # are read-only intake: humans write them in OpenProject, opilot reads them,
    ...
    class Intake
      ...
      def fetch(state, project_id:, doc_ids: [])
        ...
        raise OPilot::FatalError, "no documents found for project #{project_id}" if docs.empty?
      ...
      def write_attachment_index(dir, entries, unconvertible)
        lines = ["# Intake attachments", "",
                 "Generated by opilot. Converted from the OpenProject documents listed below.", ""]
        # Linked, not "#118": this file is committed to the spec branch, so GitHub
        # renders it and would autolink a bare id to an unrelated PR.
        entries.each { |e| lines << "- #{Helpers.document_link(@ctx, e["id"], e["title"])} (`#{e["file"]}`)" }
        ...
        if unconvertible.any?
          ...
          unconvertible.each do |u|
            lines << "- #{Helpers.document_link(@ctx, u["document"])} `#{u["file"]}` — #{u["reason"]}"
          end
        else
