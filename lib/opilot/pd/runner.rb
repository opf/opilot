  ...
  module PD
    ...
    class Runner
      ...
      def init(*args)
        ...
        puts ""
        puts "  Next: ./opilot pd intake #{project_id} <change-id> [--doc-id <id>]"
        data
      end
      ...
      def intake(*args)
        ...
        unless store.initialized?
          raise OPilot::FatalError, "no spec store for #{repo.name} yet — run `./opilot pd init #{project_id}` first"
        end
        ...
        puts ""
        puts "  Next: ./opilot pd propose #{change_id}"
        result
      end
      ...
      # reviews the decomposition, and revisions come from `@opilot` comments on
      ...
      # is undoable (opilot's HTTP client has no DELETE verb, and OpenProject
      ...
      def generate_wp(*args)
        ...
        raise OPilot::FatalError, no_sections_message(change_id) if sections.empty?
        ...
        raise OPilot::FatalError, "could not create the parent #{@ctx.pd_parent_type} work package" unless parent
      ...
      def implement(*args)
        ...
        rescue OPilot::FatalError, Harness::Error => e
      ...
      def report_publish_identity
        ...
        puts "  ⚠ github   the token is missing #{missing.join(", ")} — see the README on " \
             "why opilot needs public_repo, workflow and gist."
      ...
      def require_intake!(state, change_id)
        unless state.store.initialized?
          raise OPilot::FatalError,
                "no spec store for #{state.repo.name} yet — run `./opilot pd init <project-id>` first"
        end
        return if state.intake_dir.directory? && state.intake_dir.children.any?
        raise OPilot::FatalError,
              "change #{change_id} has no intake yet — run " \
              "`./opilot pd intake <project-id> #{change_id}` first"
      end
      ...
      def require_tasks!(state, change_id)
        ...
        raise OPilot::FatalError,
              "change #{change_id} has no tasks.md yet — run `./opilot pd propose #{change_id}` first"
      end

      def no_sections_message(change_id)
        "tasks.md for #{change_id} has no top-level `## ` sections, so there is nothing to " \
          "generate. Fix the proposal (comment `@opilot` on its PR) and re-run."
      end
      ...
      def require_resolved_ids!(state)
        ...
        raise OPilot::FatalError,
              ...
              "`./opilot pd init <project-id>` first"
      end
      ...
      def write_proposal(state, repo)
        ...
        raise OPilot::FatalError, "no proposal was written to #{change_dir}"
      end
      ...
      def publish_proposal(state, repo)
        ...
        puts "  Review the PR. Comment `@opilot <feedback>` on it to revise,"
        puts "  then: ./opilot pd generate-wp #{state.change_id}"
        url
      end
      ...
      def ensure_feature_wp(state, pr_url)
        ...
        unless project_id && type_id
          puts "  ⚠ No resolved ids yet — run `./opilot pd init <project-id>`; skipping work-package creation."
          return nil
        end
      ...
      def feature_payload(state, project_id, type_id)
        summary = read_or_empty(state.working_change_dir / "proposal.md").lines.first.to_s.sub(/\A#+\s*/, "").strip
        {
          ...
          "description" => { "format" => "markdown",
                             "raw" => "Generated by opilot from the OpenSpec change `#{state.change_id}`." },
          "_links"      => {
      ...
      def child_payload(state, section, ids, parent_wp)
        ...
        body  = +"Generated by opilot from the OpenSpec change `#{state.change_id}`"
      ...
      def report_generated(state, parent_wp, sections, created, failed)
        ...
        if failed.any?
          ...
          puts "    Re-run `./opilot pd generate-wp #{state.change_id}` — the bound ones are skipped."
        end
        puts ""
        puts "  Next: ./opilot pd implement <wp-id>"
      end
      ...
      def transition!(wp_id, item, status_name)
        ...
        unless status
          ...
          puts "    Set OPILOT_PD_IMPLEMENTING_STATUS / OPILOT_PD_IMPLEMENTED_STATUS to match this instance."
          return
        end
      ...
      def locate_section!(wp_id, repo_name)
        ...
        raise OPilot::FatalError, unknown_wp_message(wp_id, repos)
      end
      ...
      def unknown_wp_message(wp_id, repos)
        ...
        <<~MSG.strip
          ...
          no spec to implement it from. Run `./opilot pd generate-wp <change-id>` first.
      ...
      def restore_spec_tree!(store, change_id)
        ...
        puts "  ⚠ The run edited #{strays.length} spec file(s); restoring them from the store " \
             "(opilot owns openspec/, not the implementer):"
      ...
      # Deliberately not OptionParser — opilot's other commands hand-roll their
      ...
      def parse_options(args)
        ...
        until queue.empty?
          ...
          when /\A-/      then raise OPilot::FatalError, "unknown option #{arg.inspect}"
      ...
      def require_value!(queue, flag)
        value = queue.shift
        raise OPilot::FatalError, "#{flag} needs a value" if value.nil? || value.start_with?("-")
        value
      end

      # The command list itself lives in UI#pd_commands, the one place every help
      # path reads from. A bare `./opilot pd` is a request for help, not a typo —
      ...
      def usage!(subcommand, reason = nil)
        ...
        raise OPilot::FatalError, message + UI.new(@ctx).pd_usage_text
      end
    end
  end
end
