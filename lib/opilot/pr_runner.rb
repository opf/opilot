...
module OPilot
  ...
  # conflicts), fix failing CI, address feedback newer than opilot's last action,
  ...
  #
  # gh-agent reuses it via #refresh_one for "@opilot refresh", built with
  ...
  class PrRunner
    ...
    # canonical repo isn't opilot's to write to and its refresh is discarded.
    ...
    # its WP via opilot's own state, else via the OpenProject ticket link at
    ...
    # One PR, driven by gh-agent's "@opilot refresh" trigger. The commenter
    ...
    # A pasted PR URL: refresh the WP dir opilot already tracks for it, or —
    # for a PR opilot has no state for — adopt it via the OpenProject ticket
    ...
    def refresh_url(url)
      ...
      unless repo
        log_script "#{url} — base repo #{base_repo} is not in repos.json; opilot has no clone to work in."
        return
      end
    ...
    # A WP id opilot has no local PR state for (the state was reset, or the PR
    ...
    # boundary: `pr` pushes to a PR's head branch, and that is only opilot's
    ...
    # state means its id would collide with a local one. Accepts the defanged
    # `hxxp` scheme a contributor PR uses (see Helpers#neutralize_wp_links) so
    # opilot still reads back its own PRs.
    def op_ticket_id(body)
      host = op_link_host
      return nil unless host
      head = body.to_s.lines.first(OP_TICKET_SCAN_LINES).join
      head[%r{h(?:tt|xx)ps?://#{Regexp.escape(host)}(?::\d+)?/(?:\S*?/)?(?:work_packages|wp)/(\d+|[A-Z][A-Z0-9_]*-\d+)\b}, 1]
    end
    ...
    # Comments made since opilot last acted on this PR, excluding opilot's own
    # (recorded reply ids plus anything authored by the bot login). Unlike
    # gh-agent there is no @opilot-mention or allowlist gate: the operator
    # running `pr` is the trigger, and the point is to sweep up review feedback
    # that never pinged opilot.
    ...
    def confirm_push?(wp_id, branch, head_repo)
      ...
      ping_terminal("opilot: refreshed #{wp_label(wp_id)} — ready to push")
      loop do
    ...
    def post_summary(wp_id, dir, base_repo, number, reply)
      ...
      @gh_pull.record_opilot_comment(wp_id, dir.basename.to_s, comment&.id)
    rescue => e
