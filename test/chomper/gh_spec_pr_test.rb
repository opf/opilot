...
module Chomper
  ...
  class GhSpecPrTest < Minitest::Test
    ...
    def setup
      ...
      @change.mkpath
      (@change / "pr_url.txt").write("https://github.com/op-chomper/openproject/pull/14\n")
    end
    ...
    def pr(state: "open", title: "[add-recurring-meetings] Change proposal")
      PR.new(state: state, updated_at: Time.parse("2026-08-03T10:00:00Z"),
             html_url: "https://github.com/op-chomper/openproject/pull/14", title: title,
             head: Head.new("spec/add-recurring-meetings", "sha1", RepoRef.new("op-chomper/openproject")))
    end
    ...
    def test_a_mention_yields_a_spec_intent
      gh = FakeGitHub.new(pr: pr, issue: [comment(body: "@chomper split the recurrence section")])
      ...
      assert_equal "op-chomper/openproject", intent.repo, "the PR lives in the bot's own fork"
      refute intent.reply_only, "chomper owns this branch and may push to it"
    end
    ...
    def test_the_repo_comes_from_the_tracker_not_the_prs_base
      ...
      gh = FakeGitHub.new(pr: pr, issue: [comment(body: "@chomper tweak it")])
    ...
    def test_an_acted_comment_is_not_re_triggered
      gh = FakeGitHub.new(pr: pr, issue: [comment(body: "@chomper do it")])
      p  = pull(gh)
    ...
    def test_refresh_is_not_a_command_on_a_spec_pr
      # `@chomper refresh` means "merge base + fix CI + sweep feedback", none of
      # which applies to a proposal — it is just feedback text here.
      gh = FakeGitHub.new(pr: pr, issue: [comment(body: "@chomper refresh the tasks list")])
    ...
    def test_gh_agent_routes_a_spec_intent_to_revise_proposal
      ...
      intent = GhIntent.new(item_id: "add-x", spec_change_id: "add-x", repo_name: "openproject",
                            repo: "op-chomper/openproject", head_repo: "op-chomper/openproject",
                            pr_number: 14, pr_url: "u", branch: "spec/add-x", kind: :issue,
                            comment_id: 900, text: "@chomper split it", user_login: "thykel",
      ...
      assert_includes kwargs[:comment_section], "@chomper split it"
