  ...
  module PD
    ...
    class ResolvedIdsTest < Minitest::Test
      ...
      def setup
        ...
        @saved  = ENV.to_hash.slice("OPILOT_PD_IMPLEMENTING_STATUS", "OPILOT_PD_IMPLEMENTED_STATUS")
      end

      def teardown
        %w[OPILOT_PD_IMPLEMENTING_STATUS OPILOT_PD_IMPLEMENTED_STATUS].each do |key|
      ...
      def test_a_missing_status_fails_here_rather_than_mid_implementation
        ...
        error = assert_raises(OPilot::FatalError) { resolve(op) }
        ...
        assert_match(/OPILOT_PD_IMPLEMENTING_STATUS/, error.message)
      end
      ...
      def test_an_empty_status_name_is_not_required_to_exist
        ENV["OPILOT_PD_IMPLEMENTED_STATUS"] = ""
      ...
      def test_an_unreadable_project_is_reported_as_such
        error = assert_raises(OPilot::FatalError) { resolve(FakeOP.new(project_code: 404)) }
