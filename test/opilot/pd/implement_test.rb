  ...
  module PD
    ...
    class ImplementTest < Minitest::Test
      ...
      def test_an_unbound_id_says_what_is_bound
        error = assert_raises(OPilot::FatalError) { implement(runner, "999") }
      ...
      def test_a_status_this_instance_does_not_have_is_reported_and_skipped
        ...
        assert_match(/OPILOT_PD_IMPLEMENTING_STATUS/, out)
      ...
      def test_an_empty_status_name_turns_the_transition_off
        saved = ENV["OPILOT_PD_IMPLEMENTED_STATUS"]
        ENV["OPILOT_PD_IMPLEMENTED_STATUS"] = ""
        implement(runner, "501")
        assert_equal %w[7], @op.status_ids
      ensure
        saved.nil? ? ENV.delete("OPILOT_PD_IMPLEMENTED_STATUS") : ENV["OPILOT_PD_IMPLEMENTED_STATUS"] = saved
      end

      def test_it_needs_at_least_one_id
        error = assert_raises(OPilot::FatalError) { implement(runner) }
