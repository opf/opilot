...
module OPilot
  class UpstreamGhPullTest < Minitest::Test
    ...
    def test_yields_reply_only_intent_for_an_allowlisted_mention
      gh = pull(issue: [issue_c(id: 1, body: "@opilot can you review this?", login: "thykel", at: "2026-06-20T09:00:00Z")])
    ...
    def test_failing_ci_is_cached_for_the_review
      gh = pull(issue: [issue_c(id: 1, body: "@opilot why is CI red?", login: "thykel", at: "2026-06-20T09:00:00Z")],
    ...
    def test_green_ci_writes_no_cache
      gh = pull(issue: [issue_c(id: 1, body: "@opilot thoughts?", login: "thykel", at: "2026-06-20T09:00:00Z")],
                checks: [green_check])
      gh.poll_intents("2026-06-01T00:00:00Z")
    ...
    def test_ci_is_not_read_for_an_off_allowlist_mention
      gh = pull(issue: [issue_c(id: 9, body: "@opilot do it", login: "rando", at: "2026-06-20T09:00:00Z")],
                checks: [failing_check])
    ...
    def test_search_query_scopes_to_repo_cutoff_and_the_bot_login
      gh = pull(issue: [issue_c(id: 1, body: "@opilot-bot hi", login: "thykel", at: "2026-06-20T09:00:00Z")])
      gh.poll_intents("2026-06-01T00:00:00Z")
    ...
    def test_search_excludes_the_bots_own_prs
      gh = pull
      gh.poll_intents("2026-06-01T00:00:00Z")
      assert_includes @github.searches.first, "-author:opilot-bot",
                      "opilot's own PRs are GhPull's territory — the upstream scanner must not re-handle them"
    end

    def test_the_bots_own_pr_is_skipped_even_when_the_search_returns_it
      gh = pull(pr_author: "opilot-bot",
                issue: [issue_c(id: 1, body: "@opilot refresh", login: "thykel", at: "2026-06-20T09:00:00Z")])
    ...
    def test_upstream_prs_are_not_tracked_unless_explicitly_opted_in
      # No ordinary agent run should reach outside opilot's own PRs: without
      # OPILOT_TRACK_UPSTREAM_PRS there is not even a search.
      @ctx.track_upstream = false
      gh = pull(issue: [issue_c(id: 1, body: "@opilot hi", login: "thykel", at: "2026-06-20T09:00:00Z")])
      assert_empty gh.poll_intents("2026-06-01T00:00:00Z")
    ...
    def test_disabled_without_an_allowlist
      ...
      gh = pull(issue: [issue_c(id: 1, body: "@opilot hi", login: "anyone", at: "2026-06-20T09:00:00Z")])
      assert_empty gh.poll_intents("2026-06-01T00:00:00Z")
      assert_equal 0, gh.scanned_count
