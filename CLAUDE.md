...
| `publish.rb` | Pushes branch to the bot's fork via git credential helper; as the contributor opens the draft PR on the fork itself (off the upstream queue, fork base synced to upstream first), as the maintainer opens a same-repo PR on upstream — via Octokit |
...
4. **Publish** — chomper has **two GitHub identities**, and the command determines which one publishes (`Publish.new(ctx, as:)`): the **contributor** (`GITHUB_CONTRIBUTOR_TOKEN`, a dedicated bot account with no access to the canonical repos) and the **maintainer** (`GITHUB_MAINTAINER_TOKEN`, an account with push access). The agent loops always publish as the contributor; terminal `ship` derives its identity from the configured tokens (maintainer when `GITHUB_MAINTAINER_TOKEN` is set, contributor otherwise); operations on an existing PR (`pr` refresh, gh-agent follow-ups) need no identity choice — the push goes to the PR's head repo with whichever identity owns it (`PrRunner#push_client`). Commits are authored by the identity that publishes (`Helpers.adopt_github_author!(token)`). As the **contributor**, the branch is pushed to the bot's fork (`Clients::GitHub#ensure_fork`) and the draft PR is opened **on the fork itself** — same-repo, head `fork_owner:branch`, base the fork's own `base` branch (e.g. `dev` for openproject, `main` for the satellites) — so it never enters the upstream repo's PR queue; a reply with the PR link(s) is posted to the WP. Before opening, the fork's base branch is synced level with upstream (`Clients::GitHub#sync_fork_branch` → GitHub's `merge-upstream`) so the fork-hosted PR is diffed against a current base and shows only the fix (best-effort — a diverged fork just leaves some base drift showing). `maintainer_can_modify` is `false` (a same-repo PR 422s on the flag). Right after creation the PR body is patched with an **overtake note** — `gh overtake <pr-url>`, linking the README's "Taking over a chomper PR" alias setup — telling maintainers how to promote the PR to a real upstream PR under their own account (the fork PR can't run secret-gated CI, and the note passes the PR *URL* since a bare number wouldn't resolve against the maintainer's canonical repo) (`Publish#add_overtake_note`; the URL only exists post-create, hence the follow-up `update_pr_body` edit; best-effort, and skipped for maintainer PRs). `Publish#open_pr` takes the target `Repo` and drives upstream/base/worktree from it. As the **maintainer** it skips the fork and pushes straight to the canonical repo, opening a same-repo PR **on upstream** (head `upstream_owner:branch`) — also `maintainer_can_modify: false`. The one push-safety rule is **target-based, not mode-based**: any push whose target is a registry upstream (`Helpers#canonical_repo?`) is gated on an interactive terminal yes (`Helpers#confirm_canonical_push?`, shared by `Publish#open_pr`, gh-agent's `push_followup`, and the `pr` refresh); a decline skips the push (the commit stays in the clone), and a non-interactive stdin declines rather than pushing unconfirmed — so the unattended agent loops can never land anything on a canonical repo. The `protected_branch?` guard (`dev`/`main`/`master`/`release*`) is the backstop for maintainer pushes.
...
| `GITHUB_CONTRIBUTOR_TOKEN` | Token for the **contributor identity** — a dedicated bot account (not the operator) that is **not a collaborator on the canonical repo**. chomper forks as the bot, pushes branches there, and opens the draft PR on the fork itself (against the fork's own base branch, kept synced with upstream) — so nothing lands in the upstream PR queue until a maintainer overtakes it. Used by the agent modes (always) and by `ship` when no maintainer token is set. Prompted by the setup wizard; use a classic token with the `public_repo`, `workflow` and `gist` scopes (the account's lack of canonical-repo access is what enforces isolation — `workflow` is needed because a fix branch may touch upstream's `.github/workflows/*` files, which GitHub rejects from a classic PAT without that scope; `gist` lets chomper attach each WP's full plan as a secret gist linked from the PR, and the link is simply skipped if the scope is absent). Must be a personal/user account |
