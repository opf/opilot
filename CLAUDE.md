...
| `CHOMPER_PR_MODE` | Optional; `fork` (default) or `direct`. `fork` pushes the fix branch to the bot's fork and opens a cross-repo PR, so the token never needs write access to the canonical repo. `direct` pushes the branch straight to the canonical repo and opens a same-repo PR — requires the token to have push access there, and gives up the fork's isolation. To compensate, every direct-mode push (ship, gh-agent follow-up/CI fix, `pr` refresh) is gated on an interactive terminal confirmation that even `AUTO_PLAN_APPROVAL` can't bypass (a non-interactive stdin declines); the protected-branch guard + human-gated merge remain the backstop. The PR is always a draft; merging still needs a maintainer |
