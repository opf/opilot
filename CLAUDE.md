...
4. **Publish** — For each chosen repo that actually changed, the branch is pushed to the **bot's fork** (`Clients::GitHub#ensure_fork`), commits authored by the bot (`Helpers.adopt_github_author!`), a draft PR opened against that repo's upstream and `base` branch (e.g. `dev` for openproject, `main` for the satellites) with a cross-repo head (`fork_owner:branch`) and `maintainer_can_modify: true`, and a reply posted to the WP with the PR link(s). Right after creation the fork-mode PR body is patched with an **overtake note** — `gh overtake <number>`, linking the README's "Taking over a chomper PR" alias setup — telling maintainers how to re-publish the PR under their own account, since fork PRs can't run secret-gated CI (`Publish#add_overtake_note`; the number only exists post-create, hence the follow-up `update_pr_body` edit; best-effort, and skipped in direct mode where there's no fork limitation). `Publish#open_pr` takes the target `Repo` and drives upstream/base/worktree from it. With `CHOMPER_PR_MODE=direct` (`Context#direct_pr?`) it skips the fork and pushes straight to the canonical repo, opening a same-repo PR (head `upstream_owner:branch`) with `maintainer_can_modify: false` (GitHub 422s on a same-repo PR otherwise); this requires the token to have push access there. In direct mode **every push is gated on an interactive terminal yes** (`Helpers#confirm_direct_push?`, shared by `Publish#open_pr`, gh-agent's `push_followup`, and the `pr` refresh, where it also disables the `AUTO_PLAN_APPROVAL` bypass); a decline skips the push (the commit stays in the clone), and a non-interactive stdin declines rather than pushing unconfirmed. The gate is also **target-aware**: any push whose target is a registry upstream (`Helpers#canonical_repo?`) prompts regardless of mode, so a PR shipped in direct mode — whose head branch lives on the canonical repo — stays gated even under fork mode. Direct is a **script-mode option only**: the agent commands (`agent`/`op-agent`/`gh-agent`) call `Context#force_fork!` at startup (`CLI#force_fork_publishing`), pinning publishing to fork for the whole run — an unattended loop can't provide the interactive yes that direct mode's safety depends on. The `protected_branch?` guard (`dev`/`main`/`master`/`release*`) is the backstop in this mode.
...
| `CHOMPER_PR_MODE` | Optional; `fork` (default) or `direct`. `fork` pushes the fix branch to the bot's fork and opens a cross-repo PR, so the token never needs write access to the canonical repo. `direct` pushes the branch straight to the canonical repo and opens a same-repo PR — requires the token to have push access there, and gives up the fork's isolation. To compensate, every direct-mode push (ship, `pr` refresh) is gated on an interactive terminal confirmation that even `AUTO_PLAN_APPROVAL` can't bypass (a non-interactive stdin declines); the protected-branch guard + human-gated merge remain the backstop. **Script mode only**: the agent commands ignore `direct` (with a startup notice) and always publish via the fork (`Context#force_fork!`) — and any push targeting a registry upstream still prompts regardless of mode. The PR is always a draft; merging still needs a maintainer |
